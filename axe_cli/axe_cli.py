...
@axe.command()
@click.option(
    "--in", "input_path", type=click.Path(), help="Set default input directory"
)
@click.option(
    "--out", "output_path", type=click.Path(), help="Set default output directory"
)
...
def path(input_path, output_path, show):
    ...
    if input_path:
        ...
        if not input_path.is_dir():
            console.print(
                f"[bold red]Error:[/bold red] Input path must be a directory: {input_path}"
            )
            sys.exit(1)

        config.set("input_path", str(input_path))
        console.print(
            f"[bold green]✓[/bold green] Input path set to: [cyan]{input_path}[/cyan]"
        )
    ...
    if output_path:
        ...
        console.print(
            f"[bold green]✓[/bold green] Output path set to: [cyan]{output_path}[/cyan]"
        )

    if not input_path and not output_path and not show:
        console.print(
            "[yellow]No options provided. Use --help for usage information.[/yellow]"
        )
...
@click.option(
    "--format",
    "-f",
    type=click.Choice(["text", "markdown", "both"], case_sensitive=False),
    default="markdown",
...
@click.option(
    ...
    "output_dir",
    type=click.Path(),
    help="Override output directory for this operation",
)
...
def stats(show, reset):
    ...
    if show:
        ...
        console.print(
            "[yellow]Use --show to display statistics or --reset to reset them[/yellow]"
        )
