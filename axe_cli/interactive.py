...
class InteractiveMenu:
    ...
    def _show_welcome(self):
        ...
        console.print()
        console.print(Panel.fit(banner, **get_panel_style("secondary")))
        console.print()
    ...
    def _chop_menu(self):
        ...
        choice = Prompt.ask(
            "Choose operation", choices=["1", "2", "3", "4", "5"], default="1"
        )
        ...
        format_choice = Prompt.ask(
            "Output format", choices=["text", "markdown", "both"], default="markdown"
        )

        # Get output directory
        output_dir = Path(self.config.get("output_path", Path.cwd() / "axe_output"))
        custom_output = Confirm.ask(
            f"Use default output path? ({output_dir})", default=True
        )
        ...
        try:
            if choice == "1":
                ...
                if not input_path.exists():
                    console.print(
                        f"[bold red]Error:[/bold red] Path does not exist: {input_path}"
                    )
                    return
                ...
                if input_path.is_file():
                    ...
                    self.converter.process_directory(
                        input_path, output_dir, format_choice
                    )
    ...
    def _path_menu(self):
        ...
        choice = Prompt.ask(
            "Choose operation", choices=["1", "2", "3", "4", "5"], default="3"
        )
        ...
        if choice == "1":
            ...
            if not path_obj.exists():
                console.print(
                    f"[bold red]Error:[/bold red] Path does not exist: {path_obj}"
                )
                return
            ...
            console.print(
                f"[bold green][/bold green] Input path set to: [cyan]{path_obj}[/cyan]"
            )

        elif choice == "2":
            path = Prompt.ask(
                "Enter output directory path", default=str(Path.cwd() / "axe_output")
            )
            path_obj = Path(path).resolve()
            ...
            console.print(
                f"[bold green][/bold green] Output path set to: [cyan]{path_obj}[/cyan]"
            )
    ...
    def _stats_menu(self):
        ...
        choice = Prompt.ask("Choose operation", choices=["1", "2", "3"], default="1")
        ...
        if choice == "1":
            ...
            if Confirm.ask(
                "Are you sure you want to reset all statistics?", default=False
            ):
